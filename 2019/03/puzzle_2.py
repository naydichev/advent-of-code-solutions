...
def main(wires):
    ...
    intersections = filter(lambda v: v is not None, [key if len(value[0]) == 2 else None for key, value in wire_map.items()])
    min_distance = None
    min_location = None

    for cross in intersections:
        distance = count_steps(cross, wire_map)
...
def count_steps(cross, wire_map):
    wires = wire_map[cross][1:]
    if len(wires) > 2:
        print(wires)

    return sum([wire[1] for wire in wires])
...
def draw_wire(wire, wire_num, wire_map):
    ...
    for part in wire:
        ...
        for i in range(distance):
            ...
            wire_map[location][0].add(wire_num)
            wire_map[location].append((wire_num, steps))
            steps += 1
