...
def main(raw):
    maze = parse(raw)
    start = [p for p in maze.keys() if maze[p].kind == START][0]

    steps = find_shortest_path(maze, start)
...
def find_shortest_path(maze, current, keys=set()):
    pk = PointWithKeys(current, frozenset(keys))
    if pk in PATHS:
        return PATHS[pk]

    reachable_keys = find_reachable_keys(maze, current, keys)
    options = []
    if not len(reachable_keys):
        # couldn't reach anything, we must have them all
        options.append(0)
    else:
        for space, distance in reachable_keys.items():
            k_copy = keys.copy()
            k_copy.add(space.value)
            options.append(
                distance + find_shortest_path(maze, space.location, k_copy)
            )
